...
int SecCamera::startRecord(bool recordHint)
{
    ...
    CHECK(ret);
        ...
        CHECK(ret);
#ifndef BOARD_USE_V4L2_ION
        fimc_v4l2_s_fmt_is(m_rec_fd, m_recording_width, m_recording_height,
                m_preview_v4lformat, (enum v4l2_field) IS_MODE_CAPTURE_VIDEO);
#endif
    }
...
int SecCamera::getExif(unsigned char *pExifDst, unsigned char *pThumbSrc, int thumbSize, bool bneedCompress)//jmq.change
{
    ...
    unsigned int exifSize;
   // if (m_camera_use_ISP) {
    if(bneedCompress){///jmq.if need compress?
